...
const VideoDetails = () => {
  ...
  useEffect(() => {
    ;(async () => {
      ...
      if (!courseId && !sectionId && !subSectionId) {
        ...
        const filteredData = courseSectionData.filter( // section fetch kryu
          (course) => course._id === sectionId
        )
        // console.log("filteredData", filteredData)
        const filteredVideoData = filteredData?.[0]?.subSection.filter( //sec ni help thi subSec mathi vid fetch kryu
  ...
  const isFirstVideo = () => {
    ...
    // section & subSec ni index = 0 hoy to te first video hse 
  ...
  const goToNextVideo = () => {
    ...
    if (currentSubSectionIndx !== noOfSubsections - 1) { //to same sec ma next vid par javanu
      const nextSubSectionId = courseSectionData[currentSectionIndx].subSection[currentSubSectionIndx + 1]._id;
      // aa subsection na vid par jao
      navigate(
        `/view-course/${courseId}/section/${sectionId}/sub-section/${nextSubSectionId}`
      )
    } 
    else {
      // next section ma first vid par javanu
  ...
  const goToPrevVideo = () => {
    ...
    if (currentSubSectionIndx !== 0) { //same section na prev vid par jao
  ...
  const handleLectureCompletion = async () => {
    ...
    if (res) {
      dispatch(updateCompletedLectures(subSectionId)) //update the state
    }
    setLoading(false)
  }
      ...
      {!videoData ? (
        ...
        >
          ...
          {videoEnded && (
            ...
            >
              {!completedLectures.includes(subSectionId) && ( // already completed mark na hoy to j batavanu button
                <IconBtn
                  disabled={loading}
