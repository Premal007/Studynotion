...
const LearningGridArray = [
  {
    order: -1, // order:-1 e tene col-span 2 jetlu apva mate lkhelu che
  ...
  return (
    <div className="grid mx-auto w-full max-w-screen-xl grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-12 p-4">
      {LearningGridArray.map((card, i) => {
        const isHighlightCard = card.order < 0;  //order = -1 vada card mate design alag che etle
        const cardBgClass =
          card.order % 2 === 1
            ? "bg-richblack-600"
            : card.order % 2 === 0
        ...
        const colSpanClass = i === 0 ? "xl:col-span-2" : ""; //first grid no col-span 2
        const colStartClass = card.order === 3 ? "xl:col-start-2" : ""; // 3rd grid no col-start-2 thi che
