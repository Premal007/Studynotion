//alternate file for navbar :- jo coure added nai hoy to te category j nai batave
...
function Navbar() {
  // slices mathi data fetch karva mate
  ...
  const matchRoute = (route) => {
    ...
    //->{ path: route } creates an object with a path property, which is what matchPath requires.
    //-> Passing just route would not match the function's expected input and will not work.
  }  
  ...
  return (
        ...
        <nav className="">
          <ul className="flex md:gap-x-6 text-richblack-25">
            {NavbarLinks.map((link, index) => (
              <li key={index}>
                {link.title === "Catalog" ? 
                // catalog vado component aa rite handle thse
                (
                  <>
                    <div
                    ...
                    >
                      ...
                      <div className="invisible absolute left-[50%] top-[50%] z-[1000] flex w-[200px] translate-x-[-50%] translate-y-[3em] flex-col rounded-lg bg-richblack-5 p-4 text-richblack-900 opacity-0 transition-all duration-150 group-hover:visible group-hover:translate-y-[1.65em] group-hover:opacity-100 lg:w-[300px]">
                            ...
                            {/* triangle vado shape che e mate :-  */}
                ...
                // catalog sivay na components mate
                (
                  <Link to={link?.path}>
                    {/* jo path undefined of null hse to te rerender j nhi kre and crash thtu bavhavse */}
                    <p
                      className={`${
    ...
    {/* Show LOGIN,SIGNUP button or not ?  */}
          {token === null && (
            <Link to="/login">
